...
const researchLeadership = [
  ...
  { label: "Student mentorship", description: "Co-supervised four Master's dissertations, four In-Company projects, six Bachelor's projects and two Individual Studies in mechatronics and control.", badge: "16 Projects Supervised", Icon: GraduationCap },
...
const categoryCounts: Record<string, number> = { All: 29, Journals: 14, "Conference Proceedings": 11, Books: 1, "Book Chapters": 2, Posters: 1, "Theses & Dissertations": 2 };
const theses = [
  ...
  return <article id="control-miniproject" className="mt-12 scroll-mt-24 border-t border-border pt-8">
    ...
    <dl className="mt-5 grid gap-6 text-sm sm:grid-cols-2">
      ...
      <div>
        ...
        <ol className="mt-2 space-y-1.5">
          <li className="text-muted-foreground"><span className="flex gap-2"><span className="font-mono text-xs font-bold text-primary">1.</span><span>Project Supervisor: <span className="font-bold text-foreground">Shouvik Chaudhuri</span></span></span><span className="mt-0.5 block pl-5 text-xs text-muted-foreground">Project conception and progress supervision</span></li>
          <li className="text-muted-foreground"><span className="flex gap-2"><span className="font-mono text-xs font-bold text-primary">2.</span><span>Video Production: <a href="https://www.linkedin.com/in/oliverjorgensen/" target="_blank" rel="noreferrer" className="font-bold text-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Oliver Vedby Jørgensen</a> with <a href="https://www.linkedin.com/in/mat%C3%ADas-ramiro-cusicanqui-bejarano-6a6707385/" target="_blank" rel="noreferrer" className="font-bold text-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Matías R. Cusicanqui</a></span></span><span className="mt-0.5 block pl-5 text-xs text-muted-foreground">Video conception, scripting, shooting and editing</span></li>
          <li className="text-muted-foreground"><span className="flex gap-2"><span className="font-mono text-xs font-bold text-primary">3.</span><span>Project Supporter: <a href="https://www.linkedin.com/in/hossein-ramezani-a07a2672/" target="_blank" rel="noreferrer" className="font-bold text-foreground hover:text-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">Hossein Ramezani</a></span></span><span className="mt-0.5 block pl-5 text-xs text-muted-foreground">Lab-in-charge (Cyber Physical Lab)</span></li>
        </ol>
      </div>
    </dl>
    <div className="mt-5 flex flex-wrap gap-2">
      ...
      <Button asChild variant="outline" size="sm"><a href="https://www.youtube.com/watch?v=hUcPqrfgWH4" target="_blank" rel="noreferrer"><Youtube className="size-4" />Project Video 2</a></Button>
      <Button asChild variant="outline" size="sm"><a href="https://lnkd.in/p/dCdxTw6V" target="_blank" rel="noreferrer"><Linkedin className="size-4" />LinkedIn Post 1</a></Button>
      <Button asChild variant="outline" size="sm"><a href="https://lnkd.in/p/d75UzJ4U" target="_blank" rel="noreferrer"><Linkedin className="size-4" />LinkedIn Post 2</a></Button>
    </div>
    <p className="mt-3 text-xs text-muted-foreground">Matías appears in both project videos.</p>
    <div className="mt-6 flex snap-x gap-3 overflow-x-auto pb-3" aria-label="Control Miniproject photographs">
      {controlMiniprojectPhotos.map(([src, alt], index) => <button key={src} type="button" onClick={() => setSelectedPhoto(index)} className="group relative w-[17rem] shrink-0 snap-start overflow-hidden rounded-md border border-border bg-card text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring sm:w-[21rem]" aria-label={`Enlarge photo ${index + 1}: ${alt}`}>
        <img src={src} alt={alt} className="aspect-video w-full object-cover transition-transform duration-300 group-hover:scale-[1.03]" />
        <span className="absolute bottom-2 right-2 rounded-md bg-background/90 px-2 py-1 font-mono text-[10px] font-bold text-foreground shadow-sm">View {String(index + 1).padStart(2, "0")}</span>
      </button>)}
    </div>
...
const courses: { code: string; text: string; url?: string; evalUrl?: string }[] = [
  { code: 'XCOS', text: 'Experimental Control Systems · MSc Mechatronics · 5 ECTS · Spring 2023–2025 · Lecturer', url: 'https://odin.sdu.dk/sitecore/index.php?a=fagbesk&id=115633&lang=en', evalUrl: '/assets/Experimental_Control_Systems_-_Teaching_Evaluations_F23-F25.pdf' },
  { code: 'EXT', text: 'Expert in Teams · BSc · 10 ECTS · Fall 2023–2025 · Teacher and Supervisor', evalUrl: '/assets/Expert_in_Teams_-_Teaching_Evaluations_E23-E25.pdf' },
...
function Teaching() { return <Section id="teaching" eyebrow="Teaching" title="Teaching & Supervision"><a href="#control-miniproject" className="group mb-8 flex flex-wrap items-center justify-between gap-3 rounded-md border border-primary/30 bg-primary/5 px-4 py-3 transition-colors hover:bg-primary hover:text-primary-foreground"><span className="flex items-center gap-2 text-sm font-bold text-primary group-hover:text-primary-foreground"><Sparkles className="size-4 shrink-0" aria-hidden="true" />Featured: The Control Miniproject</span><span className="flex items-center gap-1.5 text-xs font-semibold text-primary group-hover:text-primary-foreground">Jump to project <ChevronRight className="size-4 transition-transform group-hover:translate-x-0.5" aria-hidden="true" /></span></a><div><div className="flex flex-wrap items-center justify-between gap-3"><h3 className="font-display text-2xl">Course Teaching</h3><Button asChild variant="outline" size="sm"><a href={teachingCertAsset.url} target="_blank" rel="noreferrer"><ExternalLink className="size-4" />View Certificate</a></Button></div><div className="mt-5 grid gap-4 md:grid-cols-2">{courses.map(c=>(<div key={c.code} className="rounded-md border border-border bg-background p-5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary hover:shadow-portrait"><p className="flex items-center gap-1.5 font-mono text-xs font-bold text-primary">{c.url?<a href={c.url} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 hover:underline">{c.code}<ExternalLink className="size-3.5" aria-hidden="true" /></a>:c.code}</p><p className="mt-2 text-sm leading-6">{c.text}</p>{c.evalUrl&&<a href={c.evalUrl} target="_blank" rel="noreferrer" className="mt-3 inline-flex items-center gap-1.5 rounded-md border border-primary/30 bg-primary/5 px-2.5 py-1 text-xs font-bold text-primary transition-colors hover:bg-primary hover:text-primary-foreground"><FileText className="size-3.5" aria-hidden="true" />Teaching Evaluations</a>}</div>))}</div><p className="mt-5 text-sm text-muted-foreground">Teaching assistant: Control of Autonomous Systems (Autumn 2022) and Construct Mechatronics (Spring 2022).</p></div><div className="mt-14 border-t border-border pt-8"><h3 className="font-display text-3xl">Student Supervision</h3><p className="mt-2 text-sm text-muted-foreground">Four Master's dissertations, four In-Company projects, six Bachelor's projects and two Individual Studies.</p><div className="mt-8 grid gap-12 lg:grid-cols-2"><div><ThesisList title="Master's dissertations" items={masters}/><div className="mt-10"><ThesisList title="In Company Projects" items={inCompanyProjects}/></div></div><div><ThesisList title="Bachelor's projects" items={bachelors}/><div className="mt-10"><ThesisList title="Individual Studies" items={individualStudies}/></div></div></div><ControlMiniproject /></div><div className="mt-14 border-t border-border pt-8"><h3 className="font-display text-3xl">Examination &amp; Assessment</h3><article className="mt-5 rounded-md border border-border bg-card p-6"><p className="text-sm font-extrabold text-primary">Internal co-examiner · University of Southern Denmark</p><p className="mt-3 text-sm leading-6 text-muted-foreground">Master's courses: Adaptive and Nonlinear Control, Fault-Tolerant Control, and Statistical Signal Processing.</p></article></div></Section>; }
...
const builtInCareerMoments: CareerMoment[] = [
  ...
  { id: "poster-odense-2025", title: "Poster Presentation at Mini-Conference in Odense", tag: "2025 · SAFEMARVEL and AMCOSTAR presentation", src: posterOdenseAsset.url, alt: "Poster presentation of SAFEMARVEL and AMCOSTAR projects in Odense", posterUrl: "/assets/IMEDay_Poster_SAFEMARVEL_AMCOSTAR.pdf" },
    ...
    <div className="grid auto-rows-[220px] gap-4 md:grid-cols-3">
      {orderedMoments.map((moment, index) => (
        ...
        >
          ...
          <figcaption className="absolute inset-x-0 bottom-0 bg-overlay px-5 py-4 text-primary-foreground backdrop-blur-sm"><p className="font-display text-xl">{moment.title}</p>{moment.tag && <p className="mt-1 text-xs opacity-80">{moment.tag}</p>}{moment.caption && <p className="mt-1 text-xs opacity-70">{moment.caption}</p>}{moment.posterUrl && <a href={moment.posterUrl} target="_blank" rel="noreferrer" onClick={(event)=>event.stopPropagation()} className="mt-2.5 inline-flex items-center gap-1.5 rounded-full border border-white/40 bg-white/10 px-2.5 py-1 text-xs font-bold text-primary-foreground transition-colors hover:bg-white/25"><FileText className="size-3" aria-hidden="true" />View poster (PDF)</a>}</figcaption>
        </figure>
      ))}
    </div>
